...
#define CMD3   (33)       /* ERASE_ER_BLK_END */
...
static void deselect (void) {
    BYTE d;
    writePinHigh(SD_CS_PIN);  /* Set CS# high */
    mmc_receive(&d, 1);       /* Dummy clock (force DO hi-z for multiple slave SPI) */
}
...
static int select (void) {  /* 1:OK, 0:Timeout */
    ...
    writePinLow(SD_CS_PIN);  /* Set CS# low */
    mmc_receive(&d, 1);      /* Dummy clock (force DO enabled) */
...
static int rcvr_datablock (  /* 1:OK, 0:Failed */
    ...
    mmc_receive(buff, btr);  /* Receive the data block into buffer */
    mmc_receive(d, 2);       /* Discard CRC */
...
static int xmit_datablock (  /* 1:OK, 0:Failed */
    ...
    d[0] = token;
    spi_transmit(d, 1); /* Xmit a token */
    if (token != 0xFD) {  /* Is it data token? */
        spi_transmit(buff, 512);  /* Xmit the 512 byte data block to MMC */
        mmc_receive(d, 2);                     /* Xmit dummy CRC (0xFF,0xFF) */
        mmc_receive(d, 1);                     /* Receive data response */
...
static BYTE send_cmd (        /* Returns command response (bit7==1:Send failed)*/
    ...
    if (cmd == CMD12) mmc_receive(&d, 1);  /* Skip a stuff byte when stop reading */
...
DSTATUS disk_initialize (
    ...
    if (send_cmd(CMD0, 0) == 1) {                    /* Enter Idle state */
        if (send_cmd(CMD8, 0x1AA) == 1) {            /* SDv2? */
            mmc_receive(buf, 4);                     /* Get trailing return value of R7 resp */
